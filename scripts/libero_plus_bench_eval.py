...
def eval_libero(config) -> None:
    ...
    if use_wandb:
        ...
        wandb_name = f"{sub_dir}_eval_{cfg.eval.task_suite_name}_step{step}_libero_plus_diff{diffusion_steps}_exec{num_steps_execute}"
        wandb.init(project=wandb_project, name=wandb_name, config=config)

    local_log_filepath = eval_dir / "eval.log"
    logger = setup_logger("libero_plus_eval", local_log_filepath)
    ...
    if json_path.exists():
        ...
        logger.warning("Task classification file not found at %s", json_path)
    ...
    def task_category(task):
        raw = task_name_to_category.get(task.name, None)
        return category_name_map.get(raw, None)
    ...
    for batch_specs in chunked(specs, num_parallel_envs):
        ...
        for result in results:
            ...
            if spec.task_category:
                category_stats[spec.task_category]["total"] += 1
            if result.success:
                task_stat["successes"] += 1
                total_successes += 1
                if spec.task_category:
                    category_stats[spec.task_category]["success"] += 1
            ...
            total_sr = total_successes / total_episodes * 100 if total_episodes > 0 else 0.0
            logger.info(
                "Episode result: task=%d/%d category=%s success=%s rollout_steps=%d total_episodes=%d/%d total_successes=%d total_success_rate=%.1f%% video=%s",
                spec.task_id + 1,
                num_tasks_in_suite,
                spec.task_category,
                result.success,
            ...
            logger.info("Category success rates:")
            for cat, stats in category_stats.items():
                if stats["total"] > 0:
                    sr = stats["success"] / stats["total"]
                    logger.info("Category %s: %d/%d (%.1f%%)", cat, stats["success"], stats["total"], sr * 100)

            if use_wandb:
                wandb_log = {
                    "eval/episode": total_episodes,
                    "eval/success": int(result.success),
                    "eval/total_success_rate": total_sr,
                }
                for cat, stats in category_stats.items():
                    if stats["total"] > 0:
                        wandb_log[f"eval/category_{cat}_success_rate"] = stats["success"] / stats["total"] * 100
    ...
    for task_id in range(num_tasks_in_suite):
        ...
        logger.info(
            "Task summary: task=%d/%d task_success_rate=%.4f total_success_rate=%.4f",
            task_id + 1,
            num_tasks_in_suite,
            task_sr,
            total_sr,
    ...
    if total_episodes > 0:
        ...
        if use_wandb:
            ...
            for cat, stats in category_stats.items():
                if stats["total"] > 0:
                    final_log[f"eval/final_category_{cat}_success_rate"] = stats["success"] / stats["total"] * 100
            wandb.log(final_log)
            wandb.finish()
