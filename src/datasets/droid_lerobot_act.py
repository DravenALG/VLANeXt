...
def _read_video_frames(video_path, frame_indices):
    ...
    try:
        frames = {}
        for position, idx in enumerate(unique_indices):
            try:
                frames[idx] = _to_uint8(reader.get_data(idx))
            except IndexError as exc:
                missing = unique_indices[position:]
                raise IndexError(
                    f"Missing frames {missing} while reading {video_path}"
                ) from exc
        return frames
    finally:
        reader.close()
...
class DroidLeRobotAct(IterableDataset):
    def __init__(
        ...
        if self.data_file_cache_size <= 0:
            raise ValueError(f"data_file_cache_size must be positive, got {data_file_cache_size}")
        ...
        self._episodes = []
        self._episodes_by_file = defaultdict(list)
        self._num_records_by_file = defaultdict(int)
        self._file_ids = []
        self._data_files = {}
    ...
    def _load_metadata(self):
        ...
        for _, row in episodes_df.iterrows():
            ...
            self._episodes_by_file[file_id].append(episode_id)
            self._num_records_by_file[file_id] += sample_count
        ...
        self._file_ids = list(self._episodes_by_file)
    ...
    def _make_rng(self, *values):
        entropy = [self.seed & 0xFFFFFFFF]
        entropy.extend(int(value) & 0xFFFFFFFF for value in values)
        return np.random.default_rng(np.random.SeedSequence(entropy))

    @staticmethod
    def _rank_record_count(num_records, rank, world_size):
        if rank >= num_records:
            return 0
        return (int(num_records) - 1 - int(rank)) // int(world_size) + 1

    def worker_buffer_sizes(self, num_workers):
        """Split one per-rank buffer budget over workers that own file shards."""
        worker_count = max(1, int(num_workers))
        active_workers = min(worker_count, len(self._file_ids))
        if self.buffer_size < active_workers:
            raise ValueError(
                f"buffer_size={self.buffer_size} must be at least the number of active "
                f"Droid workers ({active_workers})."
            )
        base, remainder = divmod(self.buffer_size, active_workers)
        return tuple(base + (worker_id < remainder) for worker_id in range(active_workers))

    @property
    def num_file_shards(self):
        return len(self._file_ids)

    def _worker_file_ids(self, worker_id, num_workers):
        return self._file_ids[int(worker_id) :: int(num_workers)]

    def _iter_file_records(self, file_id, epoch, rank, world_size):
        """Yield one rank's records from a parquet shard in episode/frame order."""
        file_position = 0
        for episode_id in self._episodes_by_file[file_id]:
            episode = self._episodes[episode_id]
            valid_len = int(episode["valid_length"])
            if valid_len <= 0:
                continue

            rng = self._make_rng(epoch, episode["episode_index"])
            if self.full_sequence:
                frame_indices = np.arange(valid_len, dtype=np.int64)
            else:
                sample_count = self._get_episode_sample_count(episode["length"])
                frame_indices = np.sort(rng.choice(valid_len, size=sample_count, replace=False))
            ...
            for frame_index, view_choice in zip(frame_indices, view_choices):
                if file_position % world_size == rank:
                    yield (
                        episode_id,
                        int(frame_index),
                        EXTERIOR_VIDEO_KEYS[int(view_choice)],
                    )
                file_position += 1

    def _iter_decoded_samples(self, epoch, shard_info):
        """Randomly visit active file shards while reading each shard sequentially."""
        rank, world_size, worker_id, num_workers = shard_info
        file_ids = [
            file_id
            for file_id in self._worker_file_ids(worker_id, num_workers)
            if self._rank_record_count(self._num_records_by_file[file_id], rank, world_size) > 0
        ]
        if not file_ids:
            return

        shard_rng = self._make_rng(epoch, rank, worker_id, 1)
        order = shard_rng.permutation(len(file_ids))
        pending = iter([file_ids[int(index)] for index in order])
        active = {}
        max_active = min(self.data_file_cache_size, len(file_ids))

        def fill_active_shards():
            while len(active) < max_active:
                try:
                    file_id = next(pending)
                except StopIteration:
                    break
                active[file_id] = iter(self._iter_file_records(file_id, epoch, rank, world_size))

        fill_active_shards()
        while active:
            records = []
            while active and len(records) < self.batch_size:
                file_ids_now = tuple(active)
                file_id = file_ids_now[int(shard_rng.integers(0, len(file_ids_now)))]
                try:
                    records.append(next(active[file_id]))
                except StopIteration:
                    del active[file_id]

            if records:
                yield from self._build_batch(records)
            fill_active_shards()

    @staticmethod
    def _iter_random_indices(rng, buffer_size, random_batch_size=100):
        while True:
            yield from (int(index) for index in rng.integers(0, buffer_size, size=random_batch_size))

    @classmethod
    def _shuffle_buffer(cls, samples, buffer_size, rng):
        """LeRobot-style replacement shuffle over already decoded samples."""
        buffer = []
        random_indices = cls._iter_random_indices(rng, buffer_size)
        for sample in samples:
            if len(buffer) < buffer_size:
                buffer.append(sample)
                continue
            index = next(random_indices)
            yield buffer[index]
            buffer[index] = sample
    ...
    def _iter_shard_samples(self, epoch, shard_info):
        _, _, worker_id, num_workers = shard_info
        buffer_sizes = self.worker_buffer_sizes(num_workers)
        if worker_id >= len(buffer_sizes):
            return
        samples = self._iter_decoded_samples(epoch, shard_info)
        buffer_rng = self._make_rng(epoch, shard_info[0], worker_id, 2)
        yield from self._shuffle_buffer(samples, buffer_sizes[worker_id], buffer_rng)
    ...
    def __iter__(self):
        epoch = self._epoch
        self._epoch += 1
        samples = self._iter_shard_samples(epoch, self._get_shard_info())
        batch = []
        for sample in samples:
            batch.append(sample)
